...
if ~isempty(r)
    ...
    conn = database('srp','root','','Vendor','MYSQL','Server','localhost','PortNumber',3306);
        
    sqlquery = ['select vno from intruder where vno = ' '''' carnum '''']; 
    name = fetch(conn,sqlquery);
    S1 = char(name);
    if isempty(S1)
        S1=carnum;
        S = 'Kotturgate';
    
        sqlquery2 = ['select pno from officer where area = ' '''' S '''']; 
        name2 = fetch(conn,sqlquery2);
        S2=char(name2)
        conn1 = database('ozekisms','root','','Vendor','MYSQL','Server','localhost','PortNumber',3306);
        tablename='ozekimessageout';
        colnames={'sender','receiver','msg','status'};
        %data(1,1)={5};
        data(1,1)={'9789845324'};
        data(1,2)={S2};
        data(1,3)={S1};
        data(1,4)={'send'};
        +datainsert(conn1,tablename,colnames,data)
    end
    close(conn);
